    ...
    <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        <div class="flex items-center justify-between h-16 gap-4">
            <a href="{{ route('home') }}" class="flex items-center gap-2">
                ...
                <span class="text-xl font-semibold tracking-tight text-slate-900">RAW VIBE ツ</span>
            </a>
            ...
            <nav class="flex items-center gap-4 text-sm">
                <a href="{{ route('shop.index') }}" class="text-slate-600 hover:text-indigo-600 hidden sm:inline">Shop</a>
                ...
                @auth
                    ...
                    <div x-data="{ open: false }" class="relative">
                        ...
                        <div x-show="open" @click.outside="open = false" x-transition
                            ...
                            <a href="{{ route('dashboard') }}" class="block px-4 py-2 text-sm text-slate-700 hover:bg-slate-50">My Orders</a>
                            <a href="{{ route('profile.edit') }}" class="block px-4 py-2 text-sm text-slate-700 hover:bg-slate-50">Profile</a>
                            @if (auth()->user()->isAdmin())
                                <a href="{{ route('admin.dashboard') }}" class="block px-4 py-2 text-sm text-indigo-600 hover:bg-slate-50">Admin Panel</a>
                            @endif
                ...
                @else
                    ...
                    <a href="{{ route('register') }}" class="bg-indigo-600 text-white px-4 py-2 rounded-full hover:bg-indigo-500">Register</a>
                @endauth
            </nav>
        </div>
