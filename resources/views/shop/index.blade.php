    ...
    <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-10">
        <div class="flex items-center justify-between mb-8">
            <div>
                ...
                <p class="text-sm text-slate-500">Explore our full catalog.</p>
            </div>
        </div>
            ...
            <aside class="bg-white rounded-2xl border border-slate-200 p-6 h-max">
                <form method="GET" action="{{ route('shop.index') }}" class="space-y-5">
                    <div>
                        ...
                        <input type="search" name="search" value="{{ $search }}" placeholder="Find products..."
                    ...
                    <div class="flex gap-2">
                        <button type="submit" class="flex-1 bg-indigo-600 text-white rounded-full py-2 text-sm font-semibold hover:bg-indigo-500">Apply</button>
                        <a href="{{ route('shop.index') }}" class="flex-1 text-center border border-slate-200 rounded-full py-2 text-sm font-semibold text-slate-600 hover:bg-slate-50">Reset</a>
                    </div>
                </form>
            </aside>
